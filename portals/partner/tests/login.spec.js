...
test.describe('Partner Portal - Login', () => {
    const baseUrl = process.env.PARTNER_PORTAL_URL || 'https://dev.partner.cocloud.in';
  const validEmail = process.env.PARTNER_EMAIL || 'contact@comhard.co.in';
  const validPassword = process.env.PARTNER_PASSWORD || 'hrhk@1111';
  ...
  test('P01 - should login with correct credentials and display dashboard', async ({ page }, testInfo) => {
    ...
    // Step 1: Navigate to partner portal and verify login form is visible
    console.log('[STEP 1] Navigating to partner portal...');
  ...
  test('P02 - should login with email in uppercase', async ({ page }, testInfo) => {
    test.setTimeout(60000);
    console.log('\n=== Test P02: Login with Email in Uppercase ===');
    ...
    // Step 3: Verify login is successful (email should be case-insensitive)
    ...
    expect(isLoginFormStillVisible === false || !currentUrl.includes('/login')).toBeTruthy();
    expect(currentUrl).not.toContain('/login');
    console.log(`✓ Login successful with uppercase email (case-insensitive) - URL: ${currentUrl}`);

    await page.screenshot({ path: 'artifacts/P02-uppercase-email.png', fullPage: true });
    console.log('✓ Test P02 completed successfully');
  });
  ...
  test('P04 - should verify all dashboard elements after login', async ({ page }, testInfo) => {
    ...
    // Step 2: Verify sidebar menu (most reliable element)
    console.log('[STEP 2] Verifying sidebar menu...');
    const isSidebarVisible = await dashboardPage.dashboardSidebar.isVisible({ timeout: 10000 }).catch(() => false);
    if (isSidebarVisible) {
    ...
    // Step 3: Verify dashboard title (optional)
    console.log('[STEP 3] Verifying dashboard title...');
    const isTitleVisible = await dashboardPage.dashboardTitle.isVisible({ timeout: 10000 }).catch(() => false);
    if (isTitleVisible) {
      console.log('✓ Dashboard title is visible');
    } else {
      console.log('⚠ Dashboard title not found (this is acceptable)');
    }
