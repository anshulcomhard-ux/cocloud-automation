class DashboardPage {
  ...
  constructor(page) {
    ...
    // HTML: input[ng-reflect-name="email"] or input[type="email"] with placeholder="Email address"
    this.emailInput = page.locator('input[formcontrolname="email"], input[ng-reflect-name="email"], input[type="email"][placeholder*="Email"], input[placeholder="Email address"]').first();
    // HTML: input[ng-reflect-name="password"] or input[type="password"] with placeholder="Password"
    this.passwordInput = page.locator('input[formcontrolname="password"], input[ng-reflect-name="password"], input[type="password"][placeholder*="Password"], input#password').first();
    // HTML: button.primary-btn with text "Sign in" or button[type="submit"]
    this.signInButton = page.locator('button:has-text("Sign in"), button.primary-btn:has-text("Sign in"), button[type="submit"].primary-btn, button.primary-btn').first();
    // HTML: a.forget-pass or a[href*="forgot"]
    this.forgotPasswordLink = page.locator('a.forgot-btn:has-text("Forgot Password"), a.forgot-btn, a.forget-pass, a[href*="forgot"], a:has-text("Forgot password")').first();
    ...
    this.dashboardTitle = page.locator('p.sub.fs-5.mb-3:has-text("Dashboard"), p.sub:has-text("Dashboard & Statistics"), p.sub:has-text("Dashboard and Statistics"), h1:has-text("Dashboard"), h2:has-text("Dashboard"), h1:has-text("Dashboard and Statistics"), h2:has-text("Dashboard and Statistics"), div:has-text("Dashboard"), *:has-text("Dashboard & Statistics"), *:has-text("Dashboard and Statistics"), *:has-text("Dashboard")').first();
   ...
   * @param {string} url - The base URL of the admin portal
   */
  async goto(url) {
    ...
    // Wait for Angular component to load - try multiple selectors
    await Promise.race([
      this.page.waitForSelector('app-auth-form', { state: 'attached', timeout: 5000 }).catch(() => {}),
      this.page.waitForSelector('form', { state: 'attached', timeout: 5000 }).catch(() => {}),
      this.page.waitForTimeout(2000)
    ]);
    
    // Wait for email input with multiple selector strategies
    try {
      await this.emailInput.waitFor({ state: 'visible', timeout: 15000 });
    } catch (error) {
      // Fallback: try to find any email input
      const fallbackEmail = this.page.locator('input[type="email"], input[placeholder*="Email"], input[placeholder*="email"]').first();
      const isVisible = await fallbackEmail.waitFor({ state: 'visible', timeout: 5000 }).catch(() => false);
      if (isVisible) {
        // Update the emailInput locator to the fallback
        this.emailInput = fallbackEmail;
      } else {
  ...
  async isEmailErrorVisible() {
    try {
      // Check for error-message div near email input - try multiple selectors
      ...
      for (const selector of emailInputSelectors) {
        try {
          ...
          const emailError = emailInput.locator('..').locator('div.error-message, div.invalid-feedback, div.text-danger').first();
  ...
  async isPasswordErrorVisible() {
    try {
      // Check for error-message div near password input - try multiple selectors
      ...
      for (const selector of passwordInputSelectors) {
        try {
          ...
          const passwordError = passwordInput.locator('..').locator('div.error-message, div.invalid-feedback, div.text-danger').first();
  ...
  async getEmailErrorMessage() {
    try {
      if (await this.isEmailErrorVisible()) {
        // Try to get error message near email input - try multiple selectors
        const emailInputSelectors = [
          'input[formcontrolname="email"]',
          'input[ng-reflect-name="email"]',
          'input[type="email"]',
        ...
        for (const selector of emailInputSelectors) {
          try {
            ...
            const emailError = emailInput.locator('..').locator('div.error-message, div.invalid-feedback, div.text-danger').first();
            if (await emailError.isVisible({ timeout: 1000 })) {
              const msg = await emailError.textContent();
  ...
  async getPasswordErrorMessage() {
    try {
      if (await this.isPasswordErrorVisible()) {
        // Try to get error message near password input - try multiple selectors
        const passwordInputSelectors = [
          'input[formcontrolname="password"]',
          'input[ng-reflect-name="password"]',
          'input[type="password"]',
